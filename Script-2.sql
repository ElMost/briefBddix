...
select c.nom_categ  from categorie c where nb_points =3;

--3. Liste des villages (noms) contenant plus de 35 huttes. (4 lignes)
select v.num_village ,v.nom_village ,v.nb_huttes ,v.num_province from village v  where nb_huttes >35;
...
select distinct a.num_hab from absorber a where num_potion='1' or num_potion='3' or num_potion='4' ;
...
select h.nom  from habitant h where  num_village =1 ;
...
join fabriquer f on f.num_potion =p.num_potion where f.num_hab ='4';

--11. Liste des potions (libellés) absorbées par Homéopatix. (2 lignes)
select distinct  p.lib_potion from potion p join absorber a on a.quantite =p.num_potion 
...
select distinct  h.nom  from habitant h  join absorber a on a.num_hab  =h.num_hab 
...
select distinct h.nom, a.date_a from habitant h  
join absorber a on a.num_hab  = h.num_hab  
join  potion p  on p.num_potion =a.num_potion 
...
select distinct h.nom , count (a.quantite) from absorber a
join potion p   on p.num_potion =a.num_potion 
join habitant h on h.num_hab =a.num_hab 
group by h.nom ;
...
join trophee t  on t.num_preneur =h.num_hab
group by h.nom
having count(t.num_trophee)> (select count(t2.num_trophee) from trophee t2
join habitant h2 on h2.num_hab = t2.num_preneur where h2.nom ='Obélix');
